...
from process_ip import format_host_strings, merge_deduplicate_ips, reorder_ips_with_best_first, select_best_ip
...
def main():
    ...
    parser.add_argument("--is_in_gha", type=bool, default=False, help="指定是否在GitHub Actions中运行")
    ...
    if not args.is_in_gha:
        ...
        ips_best = select_best_ip(ips_merged)

        logging.info("格式化hosts_single字符串")
        host_single_strings = format_host_strings(ips_best)
        ...
        ips_res = reorder_ips_with_best_first(ips_merged, ips_best)
    else:
